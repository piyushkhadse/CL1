...
public:
	...
	void postorder(node* root) { //Generating labels for nodes
		if(root) {
			postorder(root->left);
	...
	void inorder(node* root) { //Displaying labelling tree
		if(root) {
			inorder(root->left);
			cout<<root->key<<"("<<root->label<<")";
			inorder(root->right);
		}
	}
	void codegen(node* root) { //Recursive function to generate code
		if(root) {
...
int main() {
	...
	t.assignroot(st.pop());
	t.postorder(temp);
	cout<<"\nInorder Traversal Labeled tree for given expression ("<<str<<"):\n";
	t.inorder(temp);
	cout<<"\n\nAssembly language code for given expression ("<<str<<"):\n";
	t.codegen(temp);
	return 0;
}
...
student@student:~$ g++ -o codeGeneration codeGeneration.cpp 
student@student:~$ ./codeGeneration
Enter the postfix notation of the expression:
ab+ecd+--

Inorder Traversal Labeled tree for given expression (ab+ecd+--) :
a(1)+(1)b(0)-(2)e(1)-(2)c(1)+(1)d(0)

Assembly language code for given expression (ab+ecd+--):
MOV e, R1;
MOV c, R2;
ADD R2,d;
SUB R1,R2;
